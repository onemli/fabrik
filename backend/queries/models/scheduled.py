...
class ScheduledTask(models.Model):
    ...
    task_type = models.CharField(
        ...
        default=TASK_TYPE_APIC_QUERY, db_index=True,
        help_text='Type of task (APIC query or system task)',
    )
    category = models.CharField(max_length=100, blank=True, null=True, help_text='Task category for UI grouping (e.g., "Storage", "Monitoring", "Snapshots")')
    is_system_task = models.BooleanField(default=False, db_index=True, help_text='System task (admin-only, managed by platform)')
    ...
    priority = models.CharField(
        ...
        default=PRIORITY_MEDIUM, db_index=True,
        help_text='Task priority (affects execution order)',
    )
    order = models.IntegerField(default=0, db_index=True, help_text='Manual order within same priority (lower executes first)')
    ...
    saved_query = models.ForeignKey(
        SavedQuery, on_delete=models.CASCADE,
        related_name='scheduled_tasks', null=True, blank=True,
        help_text='Query or template to execute (null for system tasks)',
    )

    apic_connection_ids = models.JSONField(default=list, help_text='List of APIC connection IDs to execute against')
    variable_values = models.JSONField(null=True, blank=True, help_text='Variable values for template execution')

    retry_enabled = models.BooleanField(default=False, help_text='Enable automatic retry on failure')
    retry_count = models.IntegerField(default=3, help_text='Number of retry attempts on failure')
    retry_interval_minutes = models.IntegerField(default=5, help_text='Minutes to wait between retry attempts')

    email_on_success = models.BooleanField(default=False, help_text='Send email notification on successful execution')
    email_on_failure = models.BooleanField(default=True, help_text='Send email notification on failed execution')
    email_recipients = models.JSONField(default=list, blank=True, help_text='List of email addresses to notify')

    log_retention_days = models.IntegerField(default=30, help_text='Number of days to retain execution logs')
    ...
    minute_of_hour = models.IntegerField(null=True, blank=True, help_text='Minute of the hour for hourly tasks (0-59)')
    time_of_day = models.TimeField(null=True, blank=True, help_text='Time of day for daily tasks (HH:MM)')
    day_of_week = models.CharField(
        max_length=20, choices=DAY_CHOICES, null=True, blank=True,
        help_text='Day of week for weekly tasks',
    )
    day_of_month = models.IntegerField(null=True, blank=True, help_text='Day of month for monthly tasks (1-31)')
    scheduled_datetime = models.DateTimeField(null=True, blank=True, help_text='Specific datetime for one-time tasks')
    timezone = models.CharField(max_length=50, default='UTC', help_text='Timezone for scheduled execution')
    ...
    next_run_at = models.DateTimeField(null=True, blank=True, db_index=True, help_text='Next scheduled execution time')
    ...
    def calculate_next_run(self):
        ...
        if self.frequency == self.FREQ_ONCE:
            ...
            next_run = now.astimezone(tz).replace(minute=self.minute_of_hour, second=0, microsecond=0)
            if next_run <= now:
...
class ScheduledTaskExecution(models.Model):
    ...
    apic_connection_name = models.CharField(max_length=200, blank=True, help_text='APIC connection name (cached for history)')
    ...
    error_type = models.CharField(max_length=100, null=True, blank=True, help_text='Error type if failed')
    error_message = models.TextField(null=True, blank=True, help_text='Detailed error message')
    error_traceback = models.TextField(null=True, blank=True, help_text='Error traceback for debugging')

    retry_attempt = models.IntegerField(default=0, help_text='Retry attempt number (0 = first attempt)')
    ...
    celery_task_id = models.CharField(max_length=255, null=True, blank=True, db_index=True, help_text='Celery task ID')
    ...
    def mark_as_success(self, result, result_count=None):
        ...
        self.save(update_fields=[
            'status', 'completed_at', 'result', 'result_count', 'execution_time_ms',
        ])
    ...
    def mark_as_failed(self, error_type, error_message, traceback=None):
        ...
        self._compute_execution_time()
        self.save(update_fields=[
            'status', 'completed_at', 'error_type', 'error_message',
...
class TaskManagementSettings(models.Model):
    ...
    default_retry_count = models.IntegerField(default=3, help_text='Default number of retry attempts')
    default_retry_interval_minutes = models.IntegerField(default=5, help_text='Default minutes between retry attempts')
    default_log_retention_days = models.IntegerField(default=30, help_text='Default number of days to retain execution logs')

    email_enabled = models.BooleanField(default=False, help_text='Enable email notifications globally')
    email_from_address = models.EmailField(blank=True, default='', help_text='From email address for notifications')
