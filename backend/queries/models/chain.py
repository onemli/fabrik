...
class ChainExecutionJob(models.Model):
    ...
    total_iterations = models.IntegerField(default=0, help_text='Total number of iterations to execute')
    completed_iterations = models.IntegerField(default=0, help_text='Successfully completed iterations')
    ...
    source_result = models.JSONField(null=True, blank=True, help_text='Result from the source query')
    extracted_values = models.JSONField(default=list, help_text='Values extracted from source result (DNs, IPs, etc.)')
    aggregated_results = models.JSONField(default=list, help_text='Aggregated results from all iterations')
    errors = models.JSONField(default=list, help_text='List of errors encountered during execution')

    execution_time_ms = models.IntegerField(null=True, blank=True, help_text='Total execution time in milliseconds')
    avg_iteration_time_ms = models.IntegerField(null=True, blank=True, help_text='Average time per iteration in milliseconds')

    celery_task_id = models.CharField(max_length=255, null=True, blank=True, help_text='Celery task ID for this execution')
    ...
    current_stage_index = models.IntegerField(default=0, help_text='Index of the currently executing pipeline stage')
    execution_mode = models.CharField(
        max_length=20, default='chain',
    ...
    def mark_as_completed(self):
        ...
        self.save(update_fields=['status', 'completed_at', 'execution_time_ms', 'avg_iteration_time_ms'])
...
class ChainIterationResult(models.Model):
    ...
    job = models.ForeignKey(
        ChainExecutionJob, on_delete=models.CASCADE, related_name='iterations',
    ...
    query_params = models.JSONField(null=True, blank=True, help_text='Query parameters (filters, options, etc.)')
    ...
    error_type = models.CharField(max_length=100, null=True, blank=True, help_text='Error type (timeout, 403, 404, etc.)')
    ...
    response_size_bytes = models.IntegerField(null=True, blank=True, help_text='Size of response in bytes')
    ...
    def mark_as_success(self, result, result_count=None):
        ...
        self.save(update_fields=['status', 'completed_at', 'result', 'result_count', 'execution_time_ms'])
    ...
    def mark_as_failed(self, error_type, error_message):
        ...
        self._compute_execution_time()
        self.save(update_fields=['status', 'completed_at', 'error_type', 'error_message', 'execution_time_ms'])
