...
from .models import (
    Category, SavedQuery, QueryExecutionLog, ScheduledTask,
    ScheduledTaskExecution, TaskManagementSettings,
)
...
class SavedQueryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Query Data', {
            'fields': ('flow_data', 'generated_query')
        }),
        ('Permissions', {
            'fields': ('created_by', 'is_public', 'shared_with')
        }),
        ('Stats', {
            'fields': ('execution_count', 'last_executed_at', 'favorited_by', 'created_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
...
class QueryExecutionLogAdmin(admin.ModelAdmin):
    list_display = ['query', 'executed_by', 'executed_at', 'execution_time_ms', 'result_count', 'success']
...
class ScheduledTaskAdmin(admin.ModelAdmin):
    list_display = ['name', 'priority', 'status', 'created_by', 'frequency', 'next_run_at', 'execution_count']
    search_fields = ['name', 'description', 'created_by__username']
    list_filter = ['status', 'priority', 'frequency', 'created_at']
    readonly_fields = ['execution_count', 'success_count', 'failure_count', 'last_run_at', 'next_run_at', 'created_at', 'updated_at']
    ...
    fieldsets = (
        ...
        ('Connections & Variables', {
            'fields': ('apic_connection_ids', 'variable_values')
        }),
        ('Schedule', {
            'fields': ('frequency', 'minute_of_hour', 'time_of_day', 'day_of_week', 'day_of_month', 'scheduled_datetime', 'timezone')
        }),
        ('Retry & Notifications', {
            'fields': ('retry_enabled', 'retry_count', 'retry_interval_minutes', 'email_on_success', 'email_on_failure', 'email_recipients')
        }),
        ('Status & Stats', {
            'fields': ('status', 'log_retention_days', 'execution_count', 'success_count', 'failure_count', 'last_run_at', 'next_run_at', 'created_at', 'updated_at')
        }),
    )
...
class ScheduledTaskExecutionAdmin(admin.ModelAdmin):
    list_display = ['scheduled_task', 'apic_connection_name', 'status', 'created_at', 'execution_time_ms', 'retry_attempt']
    search_fields = ['scheduled_task__name', 'apic_connection_name']
    list_filter = ['status', 'is_retry', 'created_at']
    readonly_fields = ['scheduled_task', 'created_at', 'started_at', 'completed_at', 'execution_time_ms']
    ordering = ['-created_at']
...
class TaskManagementSettingsAdmin(admin.ModelAdmin):
    list_display = ['id', 'default_retry_count', 'default_retry_interval_minutes', 'default_log_retention_days', 'email_enabled']
