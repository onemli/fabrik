...
class APICConnectionAdmin(admin.ModelAdmin):
    list_display = ['name', 'url', 'username', 'created_by', 'is_active', 'is_public', 'last_test_status', 'created_at']
    search_fields = ['name', 'url', 'username', 'created_by__username']
    list_filter = ['is_active', 'is_public', 'last_test_status', 'created_at']
    readonly_fields = ['encrypted_password', 'last_tested_at', 'last_test_status', 'last_test_message', 'created_at', 'updated_at']
    ...
    fieldsets = (
        ...
        ('Connection Details', {
            'fields': ('url', 'username', 'verify_ssl', 'timeout')
        }),
        ('Permissions', {
            'fields': ('created_by', 'is_public', 'shared_with', 'is_active')
        }),
        ('Test Results', {
