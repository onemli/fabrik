...
class APICClient:
    ...
    def __init__(self, url: str, username: str, password: str, verify_ssl: bool = True, timeout: int = 30):
        self.url = url.rstrip('/')
    ...
    def login(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_msg = response.json().get('imdata', [{}])[0].get('error', {}).get('attributes', {}).get('text', 'Login failed')
    ...
    def execute_query(self, path: str, method: str = 'GET', data: Optional[Dict] = None) -> Tuple[bool, Any, Optional[str]]:
        """
        ...
        try:
            if method == 'GET':
                ...
                response = self.session.post(url, json=data, verify=self.verify_ssl, timeout=self.timeout)
            elif method == 'PUT':
                response = self.session.put(url, json=data, verify=self.verify_ssl, timeout=self.timeout)
            elif method == 'DELETE':
            ...
            if response.status_code in [200, 201]:
                ...
                error_msg = error_data.get('imdata', [{}])[0].get('error', {}).get('attributes', {}).get('text', 'Request failed')
