    ...
    def _validate_table_definition(self, table: Dict[str, Any], idx: int) -> List[str]:
        ...
        try:
            ...
            if 'table_name' not in table:
                ...
                errors.append(f"Table {idx}: 'table_name' must be a valid identifier (alphanumeric and underscore)")
    ...
    def _validate_column_definition(
        ...
        try:
            ...
            if 'name' not in column:
                ...
                errors.append(f"Table {table_idx}, Column {col_idx}: 'name' must be a valid identifier")
            else:
                # Check for duplicate column names
                if column['name'] in column_names:
                    errors.append(f"Table {table_idx}, Column {col_idx}: Duplicate column name '{column['name']}'")
                else:
            ...
            if 'type' not in column:
                ...
                errors.append(f"Table {table_idx}, Column {col_idx}: 'type' must be one of {valid_types}")
            ...
            if 'validation' in column:
                if not isinstance(column['validation'], str):
                    errors.append(f"Table {table_idx}, Column {col_idx}: 'validation' must be a string (regex)")
                else:
                    # Test if regex is valid
                    try:
                        re.compile(column['validation'])
                    except re.error as e:
                        errors.append(f"Table {table_idx}, Column {col_idx}: Invalid regex pattern: {str(e)}")

            # Validate enum values
            if column.get('type') == 'enum':
                if 'enum_values' not in column:
                    errors.append(f"Table {table_idx}, Column {col_idx}: 'enum' type requires 'enum_values' field")
                elif not isinstance(column['enum_values'], list):
                    errors.append(f"Table {table_idx}, Column {col_idx}: 'enum_values' must be a list")
                elif len(column['enum_values']) == 0:
                    errors.append(f"Table {table_idx}, Column {col_idx}: 'enum_values' cannot be empty")
        ...
        try:
            ...
            for table_name, table_data in input_data.items():
                ...
                if not isinstance(table_mapping, dict):
                    raise DataTransformationError(f"Mapping for table '{table_name}' must be a dictionary")

                if 'target_var' not in table_mapping:
                    raise DataTransformationError(f"Mapping for table '{table_name}' missing 'target_var'")

                if 'column_mapping' not in table_mapping:
                    raise DataTransformationError(f"Mapping for table '{table_name}' missing 'column_mapping'")
            ...
            if col_type == 'text':
                ...
                if validation_mode == 'regex':
                    ...
                    if pattern_id:
                        ...
                        try:
                            pattern = RegexPattern.objects.only('pattern').get(pk=pattern_id).pattern
