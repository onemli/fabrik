...
class ColumnTemplateService:
    ...
    def apply_template(
        self, template_id, overrides: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        try:
            # Get template
    ...
    def share_template(self, template_id, user_ids: List[int], shared_by) -> None:
        try:
            ...
            logger.info(
                f"Template '{template.name}' shared with {len(users)} users by {shared_by.username}"
            )
    ...
    def _validate_column_data(self, column_data: Dict[str, Any]) -> None:
        ...
        if column_data['type'] in ['select', 'multiselect']:
            if 'enum_values' not in column_data or not column_data['enum_values']:
                raise ColumnTemplateError('Select/multiselect types must have enum_values')

    def get_popular_templates(self, limit: int = 10) -> List:
        try:
            templates = self.ColumnTemplate.objects.filter(is_public=True).order_by('-usage_count')[
                :limit
            ]
    ...
    def duplicate_template(self, template_id, user, new_name: Optional[str] = None):
        try:
