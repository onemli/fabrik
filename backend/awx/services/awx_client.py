...
class AWXClient:
    ...
    def configure(self, url: str, token: str = None, verify_ssl: bool = True, timeout: int = 30,
    ...
    def list_job_templates(self, page: int = 1, page_size: int = 50,
                          name_filter: str = None) -> Tuple[bool, Any, Optional[str]]:
        try:
    ...
    def launch_job(self, job_template_id: int, extra_vars: Dict = None,
                   inventory_id: int = None, limit: str = None,
                   tags: str = None, check_mode: bool = False,
                   credentials: list = None) -> Tuple[bool, Any, Optional[str]]:
        try:
    ...
    def relaunch_job(self, job_id: int, is_workflow: bool = False) -> Tuple[bool, Dict, Optional[str]]:
    ...
    def list_inventories(self, page: int = 1, page_size: int = 50) -> Tuple[bool, Any, Optional[str]]:
        try:
    ...
    def get_job_events(self, job_id: int, event_type: str = None) -> Tuple[bool, Any, Optional[str]]:
        try:
    ...
    def list_workflow_templates(self, page: int = 1, page_size: int = 50,
                                name_filter: str = None) -> Tuple[bool, Any, Optional[str]]:
        try:
    ...
    def get_workflow_template_survey(self, template_id: int) -> Tuple[bool, Any, Optional[str]]:
        try:
            ...
            logger.exception(f"Error getting survey spec for workflow template {template_id}: {str(e)}")
            return False, None, str(e)
    ...
    def associate_node_credential(
        ...
        try:
            url = (
                f"{self.base_url}/api/v2/workflow_job_template_nodes/"
                f"{node_id}/credentials/"
            )
            r = self.session.post(
                url, json={'id': credential_id},
    ...
    def disassociate_node_credential(
        ...
        try:
            ...
            logger.exception(
                f"Error disassociating cred {credential_id} from node {node_id}: {e}"
            )
            return False, {}, str(e)
    ...
    def launch_workflow(self, workflow_template_id: int, extra_vars: Dict = None, check_mode: bool = False, credentials: list = None) -> Tuple[bool, Any, Optional[str]]:
        try:
    ...
    def copy_workflow_template(
        self, source_id: int, new_name: str,
    ...
    def list_workflow_templates_by_prefix(
        self, prefix: str, page_size: int = 200,
        ...
        try:
            ...
            while next_url:
                r = self.session.get(
                    next_url, verify=self.verify_ssl, timeout=self.timeout,
                )
                if r.status_code != 200:
                    return False, [], f"HTTP {r.status_code}: {r.text[:200]}"
                data = r.json()
